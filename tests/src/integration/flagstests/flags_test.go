...
func TestSupportProjectPath(t *testing.T) {
	...
	projectPath := os.Getenv("GOPATH") + "/src/github.com/nuvolaris/openwhisk-wskdeploy/tests/src/integration/flagstests"
...
func TestSupportManifestYamlPath(t *testing.T) {
	wskdeploy := common.NewWskdeploy()
	manifestPath := os.Getenv("GOPATH") + "/src/github.com/nuvolaris/openwhisk-wskdeploy/tests/src/integration/flagstests/manifest.yaml"
...
func TestSupportManifestYmlPath(t *testing.T) {
	wskdeploy := common.NewWskdeploy()
	manifestPath := os.Getenv("GOPATH") + "/src/github.com/nuvolaris/openwhisk-wskdeploy/tests/src/integration/flagstests/manifest.yml"
	_, err := wskdeploy.DeployManifestPathOnly(manifestPath)
	assert.Equal(t, nil, err, "Failed to deploy based on the manifestpath")
	_, err = wskdeploy.UndeployManifestPathOnly(manifestPath)
	assert.Equal(t, nil, err, "Failed to undeploy based on the manifestpath")
...
func TestSupportManifestYamlDeployment(t *testing.T) {
	wskdeploy := common.NewWskdeploy()
	manifestPath := os.Getenv("GOPATH") + "/src/github.com/nuvolaris/openwhisk-wskdeploy/tests/src/integration/flagstests/manifest.yaml"
	deploymentPath := os.Getenv("GOPATH") + "/src/github.com/nuvolaris/openwhisk-wskdeploy/tests/src/integration/flagstests/deployment.yml"
